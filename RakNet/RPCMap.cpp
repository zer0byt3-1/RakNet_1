...
RPCNode *RPCMap::GetNodeFromIndex(RPCIndex index) {
  if ((unsigned)index < rpcSet.Size()) return rpcSet[(unsigned)index];
  return 0;
}
RPCNode *RPCMap::GetNodeFromFunctionName(int *uniqueIdentifier) {
  unsigned index;
  index = (unsigned)GetIndexFromFunctionName(uniqueIdentifier);
  if ((RPCIndex)index != UNDEFINED_RPC_INDEX) return rpcSet[index];
  return 0;
}
...
#pragma warning(disable : 4312)
  ...
  for (index = 0; index < rpcSet.Size(); index++)
    if (rpcSet[index] &&
        reinterpret_cast<int *>(rpcSet[index]->uniqueIdentifier) ==
            uniqueIdentifier)
...
void RPCMap::AddIdentifierWithFunction(int *uniqueIdentifier,
                                       void *functionPointer,
                                       bool isPointerToMember) {
#ifdef _DEBUG
  assert(rpcSet.Size() + 1 <
         MAX_RPC_MAP_SIZE);  // If this hits change the typedef of RPCIndex to
  ...
  if ((RPCIndex)existingNodeIndex !=
      UNDEFINED_RPC_INDEX)  // Insert at any free spot.
  {
    // Trying to insert an identifier at any free slot and that identifier
    // already exists The user should not insert nodes that already exist in the
    // list
#ifdef _DEBUG
//		assert(0);
#endif
  ...
  rpcSet.Insert(
      node);  // No empty spots available so just add to the end of the list
}
void RPCMap::AddIdentifierAtIndex(RPCIndex insertionIndex) {
  ...
  existingNodeIndex =
      GetIndexFromFunctionName(reinterpret_cast<int *>(insertionIndex));

  if (existingNodeIndex == insertionIndex) return;  // Already there
  ...
  if (insertionIndex < rpcSet.Size()) {
    ...
    // Insert after the end of the list and use 0 as a filler for the empty
    // spots
...
#ifdef _DEBUG
  assert(index !=
         UNDEFINED_RPC_INDEX);  // If this hits then the user was removing an
                                // RPC call that wasn't currently registered
#endif
  RPCNode *node;
  node = rpcSet[index];
  delete node;
